        ...
        with engine.begin() as con:
            con.execute(
                ...
                {"ts": datetime.now(), "symbol": symbol, "action": action, "qty": float(qty),
                 "price": float(price), "profit_pct": float(profit_pct), "profit_eur": float(profit_eur), "status": status}
            )
            ...
            for row in df.itertuples():
                con.execute(
                    ...
                    {"ts": datetime.now(), "symbol": row.symbol, "buys": int(row.buys),
                     ...
                     "total_profit": float(row.total_profit), "total_profit_eur": float(row.total_profit_eur)}
                )
...
# sincronizează și „adoptă” pozițiile existente în balanță
    ...
    for s in strategie.get("symbols", []):
        ...
        if qty > BALANCE_EPS:
            if open_db:
                ...
                print(f"[{datetime.now()}] 🔎 {s}: OPEN (DB BUY @ {buy_px}) qty={qty}")
            else:
                adopt_px = float(get_price(s))
                pozitii[s] = {"deschis": True, "pret_intrare": adopt_px, "cantitate": qty, "max_profit": 0.0}
                print(f"[{datetime.now()}] ⚠️ {s}: adopt poziția din balanță @ {adopt_px}, qty={qty}")
        else:
...
def ruleaza_bot():
    ...
    while True:
        try:
            ...
            # împărțim 100% din EUR disponibili doar între simbolurile care NU sunt deschise
            need_buy = [s for s in strat["symbols"] if not pozitii[s]["deschis"]]
            ...
            for s in strat["symbols"]:
                ...
                # BUY doar pentru simbolurile închise
                if not p["deschis"] and alloc_sum > 0 and semnal == "BUY":
                    alloc = strat["allocations"].get(s, 0.0)
                    # partea proporțională din EUR disponibili ACUM
                    ...
                    if eur_avail >= eur_target * 0.99 and pret > 0:
                        ...
                        # rezervă din pool-ul de EUR
                        eur_avail -= eur_target
                        log_trade_db(s, "BUY", qty, pret, 0.0, 0.0)
                        print(f"[{datetime.now()}] ✅ BUY {s}: {qty:.6f} @ {pret:.2f} | EUR_spent≈{eur_target:.2f}")
                    else:
                        print(f"[{datetime.now()}] ⛔ {s}: ZEUR insuficient pentru BUY (necesar≈{eur_target:.2f}€, disponibil≈{eur_avail:.2f}€)")

                # SELL pentru pozițiile deschise
                elif p["deschis"]:
